...
public class MendixMicroflowExpressionResolver implements IMxActionExpressionResolver
{
    ...
    public static final Pattern EXPRESSIONPATTERN = Pattern.compile(String.format("\\$\\(%s\\.%s\\.([a-zA-Z0-9_]+\\.[a-zA-Z0-9_]+)\\)\\s*;?", IMxConditionExpressionResolver.EXPRESSIONKEYWORD, RESOLVER_KEY));
    ...
    public String perfromAction(String expression, Object data) throws MendixAPIExecutionException
    {
        ...
        while (matcher.find())
        {
            MendixAPIRegistry.mendixMicroflowCall.call(matcher.group(1), data);
